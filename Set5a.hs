...
setName name p = MkPerson (getAge p) name

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge age p = MkPerson age (getName p)
...
study (NthYear x) = if x < 7 then NthYear (x+1) else Graduated
...
toBin :: Int -> Bin -- un commento qua
toBin = todo -- un commento qui

{- anche questo u
n commento -}

{- questa una coppia {- di commenti -} innestati -}
